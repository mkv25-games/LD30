...
class ConstructionMenuUI extends BaseUI
{
	...
	function init()
	{
		var hs = ConstructionMenuUI.SIZE / 2;
		
		createBackgroundCover();
		createBackgroundTint();
		
		titleText = cast TextUI.makeFor("construct a unit or base".toUpperCase(), 0xFFFFFF).fontSize(32).size(ConstructionMenuUI.SIZE, 40).move(0, 70);
	...
	{
		var sourceCards = Index.cardProvider.unitCards;
	...
	function positionCards()
	{
		...
		cardLayer.x = (ConstructionMenuUI.SIZE / 2) - (cardLayer.width / 2);
		cardLayer.y = (ConstructionMenuUI.SIZE / 2) - (cardLayer.height / 2) + 20;
	}
	...
	function createBackgroundCover()
	{
		...
		g.drawRect(0, 0, ConstructionMenuUI.SIZE, ConstructionMenuUI.SIZE);
		g.endFill();
