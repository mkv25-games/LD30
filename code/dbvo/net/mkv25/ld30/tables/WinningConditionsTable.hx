...
class WinningConditionsTable implements IDBVOTable
{
	...
	public function init():WinningConditionsTable
	{
		...
		var row1:WinningConditionsRow = cast index(new WinningConditionsRow(dbvos).init(1, "Short Game", "Master of Expansion", "Most territory on three worlds", "Captured three planets, and had the most territory at the end of a round."));
		var row3:WinningConditionsRow = cast index(new WinningConditionsRow(dbvos).init(3, "Medium Game", "All your base are belong to us", "Captured all enemy bases", "Captured all bases of all your enemies at the end of a round."));
		var row4:WinningConditionsRow = cast index(new WinningConditionsRow(dbvos).init(4, "Long Game", "War, War Never Changes, War Never Ends", "Destroyed all enemy units", "Destroyed or captured every single last enemy unit and base."));
